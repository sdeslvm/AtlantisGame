...
struct AtlantisColorUtility {
    static func atlantisColor(fromHex hexString: String) -> Color {
    ...
    static func atlantisUIColor(fromHex hexString: String) -> UIColor {
        let sanitizedHex = hexString.trimmingCharacters(in: .alphanumerics.inverted)
        var colorValue: UInt64 = 0
        Scanner(string: sanitizedHex).scanHexInt64(&colorValue)
...
struct AtlantisGameInitialView: View {
    private var atlantisGameResourceURL: URL { URL(string: "https://atlantisgame.top/get")! }
    
    var body: some View {
        ZStack {
            Color(atlantisHex: "#A51D09")
                .ignoresSafeArea()
            AtlantisEntryScreen(loader: .init(resourceURL: atlantisGameResourceURL))
        }
    }
}
...
extension Color {
    init(atlantisHex hexValue: String) {
