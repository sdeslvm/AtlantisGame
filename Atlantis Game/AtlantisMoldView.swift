...
protocol AtlantisWebLoadable: AnyObject {
    var state: AtlantisWebStatus { get set }
    func setConnectivity(_ available: Bool)
}

/// Протокол для мониторинга прогресса загрузки
protocol AtlantisProgressMonitoring {
...
final class AtlantisWebLoader: NSObject, ObservableObject, AtlantisWebLoadable, AtlantisProgressMonitoring {
    // MARK: - Свойства
    
    @Published var state: AtlantisWebStatus = .standby
...
extension AtlantisWebLoader: WKNavigationDelegate {
